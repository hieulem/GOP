function [allGis,tlevelone]=Computeallgisatveryhighestlevel(Gif,howmanyframes,upwhichlevel,keephighestlevels)

%This function is the same as Computeallgis but it implements
%keephighestlevels instead of keepminprob
...
%keephighestlevels is used to keep the value of the probability that two
%trajectories are clustered together at the highest available level
if ( (~exist('keephighestlevels','var')) || (isempty(keephighestlevels)) )
    keephighestlevels=false;
end
...
highestleveldone=zeros(noTracks); %keeps track of highest level observed
    %higher levels replace the values, same values are accumulated

for g=whichGis
%     donehighestlevel=false(noTracks);
    ...
    for l=noLevels:-1:1
        ...
        for k=1:numel(r)
            ...
            if ( (~keephighestlevels) || (highestleveldone(correspondences(r(k)),correspondences(c(k)))==l) )
                %update P
                Pone=allGis.P(correspondences(r(k)),correspondences(c(k))); %accumulated value
                Ptwo=Gif.Gis{g}.P{l}(r(k),c(k)); %new P to be accumulated
...
%                 donehighestlevel(correspondences(r(k)),correspondences(c(k)))=true;
%                 donehighestlevel(correspondences(c(k)),correspondences(r(k)))=true;
            else
                if (highestleveldone(correspondences(r(k)),correspondences(c(k)))<l)
                    Pp=Gif.Gis{g}.P{l}(r(k),c(k)); %new P
                    highestleveldone(correspondences(r(k)),correspondences(c(k)))=l;
                    highestleveldone(correspondences(c(k)),correspondences(r(k)))=l;
                else
                    Pp=allGis.P(correspondences(r(k)),correspondences(c(k))); %accumulated value
                end
            end
...
function Previousfunctionpart()

            if ( (keephighestlevels) && (donehighestlevel(correspondences(r(k)),correspondences(c(k)))) )
                Pp=allGis.P(correspondences(r(k)),correspondences(c(k)));
            else
                %update P
                Pone=allGis.P(correspondences(r(k)),correspondences(c(k))); %accumulated value
                Ptwo=Gif.Gis{g}.P{l}(r(k),c(k)); %new P to be accumulated
