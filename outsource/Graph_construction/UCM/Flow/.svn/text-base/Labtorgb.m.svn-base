function [R, G, B] = Labtorgb(L, a, b, printonscreen)
...
if ( (nargin==1) || (~exist('a','var')) || (isempty(a)) || (~exist('b','var')) || (isempty(b)) )
  b = L(:,:,3);
  a = L(:,:,2);
  L = L(:,:,1);
end
...
if (printonscreen)
    Init_figure_no(301), imagesc(R), title('R channel');
    Init_figure_no(302), imagesc(G), title('G channel');
    Init_figure_no(303), imagesc(B), title('B channel');
end
...
if (printonscreen)
    Init_figure_no(305), imshow(R), title('RGB colour image');
end
