...
%TODO: program n_size to be the minimum size, not the required size
...
%               distances smaller than epsilon are excluded
%
...
elseif (mode == 2)
     if (strcmp(n_fcn,'k'))
         if (K~=0)
             ...
             Di = zeros(N*K,1);      Dj = zeros(N*K,1);       Ds = zeros(N*K,1); %upper bound for the required memory
             counter = 0; 
             [a,b,c] = find(D); 
             tic; 
             for i=1:N
         ...
         end %when K==0 D does not have to be changed
     elseif (strcmp(n_fcn,'epsilon'))
         D =  D.*(D<=epsilon); 
     end
     D = max(D,D');    %% Make sure distance matrix is symmetric
     ...
     for i=1:N
         ...
         if (strcmp(n_fcn,'k'))
             ...
             counter = counter+(K+1); 
                     ...
                     Di(counter+(1:Ki),1) = i; 
                     Dj(counter+(1:Ki),1) = b(1:Ki); 
                     Ds(counter+(1:Ki),1) = c(1:Ki); 
                     counter = counter+Ki;
                end
                if (counter<N*(K+1))
                    fprintf('%d elements entered into the sparse matrix instead of %d\n',counter,N*(K+1));
...
if ((mode==1) && (use_dijk == 0))
     ...
     D = dijkstra(D, landmarks); %D is not sparse any more here
end
...
[tmp, comp_order] = sort(size_comps,'ascend');  %% sort connected components by size
...
h = real(diag(val)); 
[foo,sorth] = sort(h,'ascend');  sorth = sorth(end:-1:1); 
