...
%F   n_size = 7 corresponds to only keeping 8 neighbours (assuming each
%    point is the closest to itself, and therefore kept)
%F   n_size = 0 corresponds to assuming that the D matrix has already
%    selected the neighbours, therefore no neighbours are discarded
%
...
%F       if the passed D had the neighbourhood already defined (n_size=0)
%        E is the same as the used spanning tree
%
...
%F indicates modification to original code
...
if strcmp(n_fcn,'k')
    if (K~=0)
         [tmp, ind] = sort(D,1,'ascend'); %F specifies paramters of sorting
         for i=1:N %F sorts vecrtically, sets to Inf horizontally, operation is ok for symmetric matrices
              D(i,ind((2+K):end,i)) = INF; %K = 7 corresponds to only keeping 8 neighbours (assuming each point is the closest to itself and therefore kept)
         end
    end
elseif strcmp(n_fcn,'epsilon')
%      warning off    %% Next line causes an unnecessary warning, so turn it off
%      D =  D./(D<=epsilon); %set to Inf all the elements in D greater than epsilon
     D(D>epsilon)=Inf; %F this avoids warning and could be faster
%      D = min(D,INF); %This may be use to eliminate NaNs
     D(isnan(D))=Inf; %F this avoids warning and could be faster
%      warning on
end
