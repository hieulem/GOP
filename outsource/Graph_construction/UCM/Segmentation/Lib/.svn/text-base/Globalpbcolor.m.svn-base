function [gPb_orient, gPb_thin, textons] = Globalpbcolor(imgFile, colorFile, outFile, rsz, count, saveworkimages, printthetextonscreen)
% syntax:
...
% weights = [weights, 0.0025, 0.0025, 0.0028, 0.0025, 0.0025, 0.0028]; %gPb in the experiments
% weights = [weights, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001]; %g1Pb in the experiments
weights = [weights, 0.0035, 0.0035, 0.0038, 0.0035, 0.0035, 0.00385];
% weights = [weights, 0.0035, 0.0035, 0.0038, 0.0035, 0.0035, 0.0038];
% [bg1 bg2 bg3 cga1 cga2 cga3 cgb1 cgb2 cgb3 tg1 tg2 tg3 sPb cga1c cga2c cga3c cgb1c cgb2c cgb3c]

%% mPb with color flow image
[mPb, mPb_rsz, bg1, bg2, bg3, cga1, cga2, cga3, cgb1, cgb2, cgb3, tg1, tg2, tg3, textons, cga1c, cga2c, cga3c, cgb1c, cgb2c, cgb3c] = ...
    Multiscalepbcolor(im, imcolor, rsz, printthetextonscreen);
t = toc;
...
for o = 1 : size(gPb_orient, 3),
    ...
    a1c = weights(14)*cga1c(:, :, o);
    a2c = weights(15)*cga2c(:, :, o);
    a3c = weights(16)*cga3c(:, :, o);

    b1c = weights(17)*cgb1c(:, :, o);
    b2c = weights(18)*cgb2c(:, :, o);
    b3c = weights(19)*cgb3c(:, :, o);

    gPb_orient(:, :, o) = l1 + a1 + b1 + t1 + l2 + a2 + b2 + t2 + l3 + a3 + b3 + t3 + sc + a1c + b1c + a2c + b2c + a3c + b3c;
end
