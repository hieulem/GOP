...
int indexwithtree(int i,int j,int f,int dimI,int dimJ)
{
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
    ...
    //mexPrintf("index %d, labelledvideo(5,2,3) %f\n",indexwithtree(5,2,3,dimIi,dimIj),labelledvideo[indexwithtree(5,2,3,dimIi,dimIj)]);
    for (f=0;f<nointerestframes;f++)
    {
        ...
        for (i=0;i<dimIi;i++)
        {
            ...
            for (j=0;j<dimIj;j++)
            {
                uj=min(dimIj-1,j+1);
                //if (i==0)
                    //mexPrintf("Value test (%d,%d,%d) %d %d\n", i, j, ff, int(labelledvideo[indexwithtree(i,j,ff,dimIi,dimIj)]), int(labelledvideo[indexwithtree(ui,j,ff,dimIi,dimIj)]));
                //labelindex=indexwithtree(i,j,f,dimIi,dimIj);
                //ucmindex=indexwithtwo(i,j,2*dimIi+1);
                if ( int(labelledvideo[indexwithtree(i,j,ff,dimIi,dimIj)]) != int(labelledvideo[indexwithtree(ui,j,ff,dimIi,dimIj)]) )
                    for (k=-1;k<=1;k++)
                        voneucm[indexwithtwo(2*(i+1),2*(j+1)-1+k,2*dimIi+1)]=voneucmconst[indexwithtwo(2*(i+1),2*(j+1)-1+k,2*dimIi+1)] + int(1);
                if ( int(labelledvideo[indexwithtree(i,j,ff,dimIi,dimIj)]) != int(labelledvideo[indexwithtree(i,uj,ff,dimIi,dimIj)]) )
                    for (k=-1;k<=1;k++)
