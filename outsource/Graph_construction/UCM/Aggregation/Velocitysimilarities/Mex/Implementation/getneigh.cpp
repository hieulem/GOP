  ...
  // I. INITIATE
   ...
   if(num_alabel > 1)
   {
       for( i =0 ; i< num_alabel; i++)
       {
            ...
            bool_all_spx[thelabel-1] = true;     //changed from j to j-1
            
            for(j=0; j< num_conrr; j++)
            {
                if(conrr[j] == thelabel)
                {
                   bool_all_spx[(int) concc[j]-1] = true;           //changed from j to j-1
                }
                if(concc[j] == thelabel)
                {
                  bool_all_spx[(int) conrr[j]-1] = true;  //changed from j to j-1
                }
            }
//!!!!!!!!!remove the original labels (alabel) from the neighlabel!!!!!!!!!
             bool_all_spx[thelabel-1] =false;                //changed from j to j-1
       } 
   }
            ...
            if ( (* thedepth>2) && (d< * thedepth) )
            {
                ...
                { 
                    if(num_alabel > 1)
                    {
                       ...
                       {
                            thelabel = (int)alabel[i];
                            bool_newneighlabels[thelabel-1] = false;   //changed from j to j-1
                       }
                    }
                   else if(num_alabel == 1)
                   {
                        bool_newneighlabels[((int) *alabel) -1] = false;
                   }
                }
            }
   }
