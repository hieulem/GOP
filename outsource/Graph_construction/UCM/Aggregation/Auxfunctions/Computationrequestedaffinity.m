...
switch (anaffinity)
    ...
    case 'ltt'
        ...
        if (~completion) %This could also be done with stt
            if (~Isaffinityrequested(theset,'ltt','stt','stm','sta')) %('vltti') this could also be inserted here, but it is not guaranteed to have connections at all frames
                isnecessary=true;
            end
        end
    ...
    case 'vltti'
        if (Isaffinityrequested(theset,'vltti'))
            isnecessary=true;
        end
