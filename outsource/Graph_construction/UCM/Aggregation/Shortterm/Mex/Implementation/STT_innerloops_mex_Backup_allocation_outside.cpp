 ...
 * Usage :
 * mex -largeArrayDims STT_innerloops_mex_Backup_allocation_outside.cpp  -I. initarrytozero.cpp initbooltozero.cpp initintarrytozero.cpp Evolveregionsfastwithfilteredflows_cpp_backup_allocation_outside.cpp Measuresimilarity_cpp.cpp 
 *
 * Command to execute:
 * STT_innerloops_mex_Backup_allocation_outside.(graphdepth,noFrames,labelledlevelvideo,multcount,maxnumberofsuperpixelsperframe,flows,mapped,sxo,syo,svo)
 */
...
//         }        
//  }
//  void indToSub2d(const int no_rows,const int no_cols, int indices, int &row_index, int &col_index)
//  {
//         row_index = (indices % no_rows);
//         col_index = (indices / no_rows)+1;
//         if ((indices % no_rows) ==0 )
...
//             col_index = (indices / no_rows);
//         }
//         if(col_index > no_cols)
//         {           
//             col_index = col_index - no_cols*((col_index-1)/no_cols); 
//         }
//  }
//  
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
   /* DECLARATIONS: */
    ...
    int dimframe_labvideo,dimrow_labvideo,dimcol_labvideo, i, start_index,dimrow,dimcol;
    bool  *bool_masksonframe, *similaritydoneone, *themask;
    int *labelsonone, *labelsontwo;  
    double  *similarityatone,importanceforprobability,*predicted_mask;
...
// pointers to extract a flow from the structure
   ...
   // memory allocations for the temp arrays
      ...
      avgxs                       = new double[dimrow_labvideo*dimcol_labvideo];
      avgys                       = new double[dimrow_labvideo*dimcol_labvideo];
      xplus                       = new double[dimrow_labvideo*dimcol_labvideo];
      xminus                      = new double[dimrow_labvideo*dimcol_labvideo];
      yplus                       = new double[dimrow_labvideo*dimcol_labvideo];
      yminus                      = new double[dimrow_labvideo*dimcol_labvideo]; 
...
//       initbooltozero(themask               ,(dimrow_labvideo*dimcol_labvideo));
      ...
      initarrytozero(avgxs  ,(dimcol_labvideo*dimrow_labvideo));
      initarrytozero(avgys  ,(dimcol_labvideo*dimrow_labvideo));
      initarrytozero(xplus  ,(dimcol_labvideo*dimrow_labvideo));
      initarrytozero(xminus ,(dimcol_labvideo*dimrow_labvideo));
      initarrytozero(yplus  ,(dimcol_labvideo*dimrow_labvideo));
      initarrytozero(yminus ,(dimcol_labvideo*dimrow_labvideo));
...
//                     initarrytozero(yplus  ,(dimcol_labvideo*dimrow_labvideo));
//                     initarrytozero(yminus ,(dimcol_labvideo*dimrow_labvideo));
                    Evolveregionsfastwithfilteredflows_cpp_backup_allocation_outside(predicted_mask,themask,tmp_flows_flows_no_Up_matrix,tmp_flows_flows_no_Vp_matrix,
                            tmp_flows_flows_no_Um_matrix,tmp_flows_flows_no_Vm_matrix,dimrow_labvideo,dimcol_labvideo,
                            avgxs,avgys,xplus,xminus,yplus,yminus);                    
                    if((int) *graphdepth >1)
                    {
