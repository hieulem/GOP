function [ML_matrix]=Getmustlinksfromucm(twlabelledlevelvideo,tmpoptions,twucm2)
...
if (Level==0) %Compute normpertrack with the pixels within the provided superpixelization
    ...
    [twlabelledlowerlevel]=Labellevelframes(twucm2,Level,noFrames); %,numberofsuperpixelsperframe
...
corrsupertracks=double(corrsupertracks);
    corrsupertracks = sparse(corrsupertracks);
    [ii,jj,vv] =find(corrsupertracks);
    s = sparse(jj,ii,vv,size(corrsupertracks,2),size(corrsupertracks,1));
    ML_matrix = triu(s*corrsupertracks,1);    
