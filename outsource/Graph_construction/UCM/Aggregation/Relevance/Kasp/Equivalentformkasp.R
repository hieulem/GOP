...
#}
smatz<-smat;
#smatz matrix contains the distance between the centers
#smatz[i,j] is the squared distance between center i and j
#smatz[i,j]= (norm(i,j))^2

#smatz is the affinity matrix
#smatz should be reweighted according to our reweighting module

#Compute the multiplicity of the clustered points, used in the reweighting
normpertrack<-matrix(0,n,1)
#dim(multiplicity)
for(i in 1:N)
{
	normpertrack[xkms$cluster[i]]<-normpertrack[xkms$cluster[i]]+1
}
...
if (F) {
	#Reweight the affinity matrix smat according to the multiplicity normpertrack
	maxnotracks<-nrow(smat)
	...
	#weights off the diagonal
	smatw<- smatw * kronecker(normpertrack,t(normpertrack),FUN="*");

	#weights on the diagonal
	for(i in 1:maxnotracks)
	{
		# smatw[i,i]=0;
		...
		smatw[i,i]=
			...
			(
				...
				-
				sum(
				    smat[i,-i] * normpertrack[-i]
				)
			) 
	}

	#Reweighted affinity assigned back to smat
	smat<-smatz
}
