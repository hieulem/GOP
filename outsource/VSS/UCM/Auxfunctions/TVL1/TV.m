...
function flow=TV(I1,I2)

%requires: path(path,'TVL1');
% path(path,'TVL1');

%commented output in warping line 91, 101, 102, in coarse_to_fine line 115
...
%I1 = double(imread('frame10.png'))/255;
%I2 = double(imread('frame11.png'))/255; 

tic
  
  % call main routine
  [flow] = coarse_to_fine(I1, I1, I2, lambda, theta, epsilon, use_edges, maxits, ...
                          max_pyramid_levels, pyramid_factor, warps, ...
                          use_structure_texture, structure_texture_sigma, structure_texture_factor);
toc

% write out color coded flow
%imwrite(uint8(flowToColor(flow)),'out.png');
