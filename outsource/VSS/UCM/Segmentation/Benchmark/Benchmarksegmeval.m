...
for i = 1:numel(iids)
    ...
    if (considervideos)
        [videodetected,videoimagename,fnames,nframes]=Detectvideo(iids,i);
    else
        videodetected=false;
%         videoimagename=iids(i).name(1:end-4);
    ...
    if (videodetected)
        if (any(strcmp(encvideos,videoimagename)))
    ...
    if ( (analyzeonevideo) && (considervideos) && (~strcmp(videoimagename,justavideo)) ) %( (analyzeonevideo) && (~strcmp(videoimagename,justavideo)) )
        continue;
    end
    if ( (analyzeonevideo) && (~considervideos) && ( (numel(videoimagename)<numel(justavideo)) || (~strcmp(videoimagename(1:numel(justavideo)),justavideo)) ) )
        continue; %tmp check for images
    end
    
    evFile1 = fullfile(outDir, strcat(videoimagename, '_ev1.txt')); %bdry
    evFile2 = fullfile(outDir, strcat(videoimagename, '_ev2.txt')); %sc
    evFile3 = fullfile(outDir, strcat(videoimagename, '_ev3.txt')); %sc
    evFile4 = fullfile(outDir, strcat(videoimagename, '_ev4.txt')); %pri,vi
    ...
    if ( (any(strcmp(bmetrics,'bdry'))) || (any(strcmp(bmetrics,'all'))) )
        use3dbdry=false;
        Evaluatesegmbdry(inFile,gtFile, evFile1, nthresh, maxDist, thinpb, use3dbdry, excludeth);
    end
    %Evaluate region benchmarks
    if ( (any(strcmp(bmetrics,'sc'))) || (any(strcmp(bmetrics,'pri'))) || (any(strcmp(bmetrics,'vi'))) || (any(strcmp(bmetrics,'all'))) )
        Evaluatesegmregion(inFile, gtFile, evFile2, evFile3, evFile4, [], nthresh, bmetrics, excludeth);
    end
end
fprintf('\n');
