function [L,a,b] = Rgbtolab(R, G, B, printonscreen)
...
if ( (nargin==1) || (~exist('G','var')) || (isempty(G)) || (~exist('B','var')) || (isempty(B)) )
  B = double(R(:,:,3));
  G = double(R(:,:,2));
  R = double(R(:,:,1));
end
...
if (printonscreen)
    Init_figure_no(301), imagesc(L), title('L channel');
    Init_figure_no(302), imagesc(a), title('a channel');
