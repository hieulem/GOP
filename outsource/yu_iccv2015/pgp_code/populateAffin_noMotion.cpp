...
static inline double round2(double x) { return (floor(x + 0.5)); } 
		...
		for (int j = 0; j < rowSize; j ++){
			...
			if (tvPr[j] > i){
				...
				if (isTemporal == 1) {
					...
					cR = nY-selfY; //y-direction offset from self
					cC = nX-selfX; //x-direction offset from self
				...
				for (int k = 0; k < spList1L; k ++){
					histI1[(mwIndex)round2(mxGetPr(inputImgG1)[(mwIndex)(mxGetPr(spList1)[k])-1])] ++;

				}
				
				spList2L = (int)mxGetM(spList2);
				for (int k = 0; k < spList2L; k ++){
					histI2[(mwIndex)round2(mxGetPr(inputImgG2)[(mwIndex)(mxGetPr(spList2)[k])-1])] ++;
				}
				...
				*/
				...
				for (int k = 0; k < spList1L; k ++){
					...
					tempH = round2(mxGetPr(hueImg1)[(mwIndex)(mxGetPr(spList1)[k])-1]);
					if (tempH == -1){
						tempH = 1;
					}
					/*
				...
				for (int k = 0; k < spList2L; k ++){
					...
					tempH = round2(mxGetPr(hueImg2)[(mwIndex)(mxGetPr(spList2)[k])-1]);
					if (tempH == -1){
						tempH = 1;
					}
				...
				if (((double)nonZeros1 > 0.3*spList1L) && ((double)nonZeros2 > 0.3*spList2L) && ((double)runningM1/spList1L > 10) && ((double)runningM2/spList2L > 10)){
					...
					for (int k = 0; k < anglesV1.size(); k ++){
						...
						histO1[(mwIndex)round2(anglesV1.at(k))-1] ++;
						if (anglesV1.at(k) < 181){
							histO1[(mwIndex)round2(anglesV1.at(k))-1+180] ++;
						}
					}
					...
					for (int k = 0; k < anglesV2.size(); k ++){
						...
						histO1[(mwIndex)round2(anglesV2.at(k))-1] ++;
						if (anglesV2.at(k) < 181){
							histO2[(mwIndex)round2(anglesV2.at(k))-1+180] ++;
						}
					}
