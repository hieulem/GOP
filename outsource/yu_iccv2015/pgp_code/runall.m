...
i = myind2sub([4,2,2],id,3);
option.useGeo= i(2);
option.dataset = i(3);  %1:segtrack,%2:chen
option.numSP = i(1); %number of superpixels to extract per frame
...
switch option.dataset
    case 1
        ...
        option.outputDIR = ['./output/Segtrack/',name,'/',num2str(option.useGeo),'/']; %output directory that saves the segmented frames in labels.
        ...
        if option.useGeo ==0
            savename= ['sBL_', num2str(option.numSP),'_',name];
            save([savepath,savename], 'thesegmentation','s');
        else
            switch gehoptions.type
                case '1d'
                    savename= ['sG1D_', num2str(option.numSP),'_',num2str(gehoptions.phi),'_',...
                        num2str(gehoptions.nGeobins),'_', num2str(gehoptions.maxGeo),'_', num2str(gehoptions.usingflow),'_',name];
                    save([savepath,savename], 'thesegmentation','s');
                case '2d'
                    savename= ['sG2D_', num2str(option.numSP),'_',num2str(gehoptions.phi),'_',num2str(gehoptions.nGeobins)...
    ...
    case 2
        ...
        if option.useGeo ==0
            savename= ['cBL_', num2str(option.numSP),'_',name];
            save([savepath,savename], 'thesegmentation','s');
        else
            switch gehoptions.type
                case '1d'
                    savename= ['cG1D_', num2str(option.numSP),'_',num2str(gehoptions.phi),'_',...
                        num2str(gehoptions.nGeobins),'_', num2str(gehoptions.maxGeo),'_', num2str(gehoptions.usingflow),'_',name];
                    save([savepath,savename], 'thesegmentation','s');
                case '2d'
                    savename= ['cG2D_', num2str(option.numSP),'_',num2str(gehoptions.phi),'_',num2str(gehoptions.nGeobins)...
                        num2str(gehoptions.nIntbins),'_', num2str(gehoptions.maxGeo),'_',num2str(gehoptions.maxInt),'_', num2str(gehoptions.usingflow),'_',name];
                    save([savepath,savename], 'thesegmentation','s');
