...
for source_frame =1:nf-1
    ...
    neighbor = pop_adjacent_matrix(sp(:,:,target_frame));
    filter = seeds_geo{target_frame}<0.1;
    neighbor = neighbor.*filter;
    numedges = numel(find(neighbor))
    edge = find(neighbor);
    [from,to] = ind2sub(size(neighbor),edge);
    edge_index = [from,to];
%     mark = zeros(1,numedges);
%     for i=1:numedges
%         fr= edge_index(i,1);
%         t = edge_index(i,2);
%         if seeds_geo{target_frame}(fr,t) > 0.2
%             mark(i) =1;
%             neighbor(fr,t) = 0; 
%             neighbor(t,fr) = 0;
%         end
