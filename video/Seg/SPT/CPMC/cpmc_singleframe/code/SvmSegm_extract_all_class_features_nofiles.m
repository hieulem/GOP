...
function D = SvmSegm_extract_all_class_features_nofiles(exp_dir, img_name, mask_type, class_feats, delete_local_feats, overwrite)
   DefaultVal({'delete_local_feats', 'overwrite'},{ 'true','false'});
   ...
   parfor i=1:numel(class_feats)
       filename = [exp_dir 'MyMeasurements/' mask_type '_' class_feats{i} '/' img_name{1} '.mat'];
       if(the_pars{i}.isbow && (~exist(filename,'file') || overwrite))
