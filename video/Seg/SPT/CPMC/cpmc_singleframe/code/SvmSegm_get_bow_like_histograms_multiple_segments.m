...
function SvmSegm_get_bow_like_histograms_multiple_segments(mask_type, measurement_type, figure_or_ground, img_names, exp_dir, codebook_selection, overwrite)
    codebook_dir = [exp_dir '/MyCodebooks/'];
    representation = 'bow';
    
    default_codebook_selection = ['kmeans_' measurement_type '_300_words.mat'];
    DefaultVal_JL('codebook_selection', 'default_codebook_selection');    
...
function mysave(filename,D)
    direc_end = find((filename == '/' | filename == '\'),1,'last');
    if ~exist(filename(1:direc_end),'dir')
        mkdir(filename(1:direc_end));
    end
    save([filename], 'D');
end
