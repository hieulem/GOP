function [F,D] = csift(I,I_name, spacing, scales, color_descriptor_type)
    if(size(I,3) == 3)
    ...
    DefaultVal_JL('*color_descriptor_type', '''csift''');

    rand('twister', twist);

    img_filename = ['./img' I_name int2str(rand*10000000000000) '.jpg'];
    ...
    output_filename = ['./csift' I_name int2str(rand*10000000000000) '.tmp'];
    path = which('csift');
